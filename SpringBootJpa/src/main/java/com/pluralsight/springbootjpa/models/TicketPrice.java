...
@NamedQuery(
        name = "TicketPrice.namedParamsFindTicketsByPricingCategoryName",
        query = "select tp from TicketPrice tp where tp.pricingCategory.pricingCategoryName = :name"
)
@NamedQuery(
        name = "findTicketsByPricingCategoryName",
        query = "select tp from TicketPrice tp where tp.pricingCategory.pricingCategoryName = ?1"
)
@NamedNativeQuery(
        name = "TicketPrice.nativeParamsFindTicketsByCategoryWithWorkshop",
...
@NamedNativeQuery(
        name = "findTicketsByCategoryWithWorkshop",
        query = "select tp.* from ticket_prices tp " +
                "left join ticket_types tt on tp.ticket_type_code = tt.ticket_type_code " +
                "left join pricing_categories pc on tp.pricing_category_code = pc.pricing_category_code " +
                "where tt.includes_workshop = true and pc.pricing_category_name = ?1",
        resultClass = TicketPrice.class
)
